---
to: "server/api/<%= h.inflection.dasherize(h.inflection.underscore(name)) %>/[id]/index.put.ts"
unless_exists: true
---
...
<% } -%>
  ...
  const { get<%= h.inflection.camelize(name) %>, update<%= h.inflection.camelize(name) %> } = use<%= h.inflection.camelize(name) %>Service(prisma);
